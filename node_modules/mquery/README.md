...
```js
var greatContacts = mquery(contactCollection).where('rating').gte(4.5).toConstructor();

// use it!
greatContacts().count(function (err, n) {
  console.log('There are %d great contacts', n);
});

greatContacts().where({ name: /^Life/ }).select('name').find(function (err, docs) {
  console.log(docs);
});
```
